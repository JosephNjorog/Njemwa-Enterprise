...
(function ($) {
    ...
    FlotTooltip.prototype.init = function (plot) {
        ...
        function plothover(event, NE, item) {
            ...
            if (item) {
                plot.showTooltip(item, that.tooltipOptions.snap ? item : NE);
                ...
                $.each(plot.getData(), function (i, series) {
                    ...
                    for (var j = 1; j < series.data.length; j++) {
                        if (series.data[j - 1][0] <= NE.x && series.data[j][0] >= NE.x) {
                            xBeforeIndex = j - 1;
                            xAfterIndex = j;
                        }
                    }
                    ...
                    var distToLine = dotLineLength(series.xaxis.p2c(NE.x), series.yaxis.p2c(NE.y), series.xaxis.p2c(pointPrev.x),
                        series.yaxis.p2c(pointPrev.y), series.xaxis.p2c(pointNext.x), series.yaxis.p2c(pointNext.y), false);

                    if (distToLine < closestTrace.distance) {

                        var closestIndex = lineDistance(pointPrev.x, pointPrev.y, NE.x, NE.y) <
                            lineDistance(NE.x, NE.y, pointNext.x, pointNext.y) ? xBeforeIndex : xAfterIndex;
                        ...
                        var pointOnLine = [
                            NE.x,
                            pointPrev.y + ((pointNext.y - pointPrev.y) * ((NE.x - pointPrev.x) / (pointNext.x - pointPrev.x)))
                        ];
        ...
        plot.setTooltipPOSition = function (NE) {
            var $tip = that.getDomElement();

            var totalTipWidth = $tip.outerWidth() + that.tooltipOptions.shifts.x;
            var totalTipHeight = $tip.outerHeight() + that.tooltipOptions.shifts.y;
            if ((NE.x - $(window).scrollLeft()) > ($(window)[that.wfunc]() - totalTipWidth)) {
                NE.x -= totalTipWidth;
            }
            if ((NE.y - $(window).scrollTop()) > ($(window)[that.hfunc]() - totalTipHeight)) {
                NE.y -= totalTipHeight;
            }

	    /* 
	       The section applies the new POSitioning ONLY if NE.x and NE.y
		...
		that.tipPOSition.x = NE.x;
		that.tipPOSition.xPrev = NE.x;
	    }
	    if (isNaN(NE.y)) {
		that.tipPOSition.y = that.tipPOSition.yPrev;
	    }
	    else {
		that.tipPOSition.y = NE.y;
		that.tipPOSition.yPrev = NE.y;
	    }
